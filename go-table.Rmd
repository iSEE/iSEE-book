...
Note that `r BiocStyle::CRANpkg("shiny")` also has a `dataTableOutput` function that is almost-but-not-quite-the-same so care must be taken to disambiguate them if both symbols are imported.
...
We generate the output by specializing the `.generateOutput()` function, using the `r BiocStyle::Biocpkg("GO.db")` package to create a table of GO terms and their definitions.
